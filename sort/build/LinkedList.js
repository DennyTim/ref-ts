...
var LinkedList = /** @class */ (function (_super) {
    ...
    LinkedList.prototype.add = function (data) {
        var node = new Node(data); //создаем ноду
        if (!this.head) { //проверка, является ли нода первой в списке
            this.head = node;
            return;
        }
        var tail = this.head; // если элемент не первый записываем его в temp
    ...
    LinkedList.prototype.swap = function (leftIndex, rightIndex) {
        var _a;
        var leftNode = this.find(leftIndex);
        var rightNode = this.find(rightIndex);
